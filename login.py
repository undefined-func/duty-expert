...
conn = sqlite3.connect("duty_expert.db", check_same_thread=False)
### Creation ############################################################
#try:conn.execute("DROP TABLE users;")
#except: pass
try:
    creation = [
        "CREATE TABLE users (username text PRIMARY KEY, fullname text, password text);"
    ]
    for c in creation: conn.execute(c)
except:pass
user_manager = um.UserManager(sql_conn=conn)
...
'''
@app.route(LOGIN_SECTION + "/settings")
def settings():
